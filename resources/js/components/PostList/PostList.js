        ...
        <div className="post-list mb-5">
            <div className="text">
                <div className="profile mb-2">
                    ...
                    <span className="username mx-2">{item.user.name || item.user.username}</span>
                </div>
                <Link to={`/${item.user.userid}/${item.slug}`} className="title">{item.short_title}</Link>
                <div className="mt-2">
                    <p>{item.desc}</p>
                    <p>{item.created_at}</p>
                    {(state.user && state.user.id === item.user.id) && (
                        <>
                            ...
                            <Link to="/delete" className="text-danger">Delete</Link>
                        </>
                    )}
                </div>
            </div>
            <div className="image text-right">
                <img src={item.image ? item.image : DEFAULT_IMAGE} className="img rounded thumbnail"/>
            </div>
        </div>
    )
}
